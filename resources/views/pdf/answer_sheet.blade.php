    ...
    @foreach($answerSheet->answer_sheet_question as $question)
        ...
        <table style="width: 100%; border-bottom: 1px solid black; margin-bottom: 10px;">
            @for($i=0;$i<$question->answer_sheet_question_answer->count();$i=$i+2)
                <tr>
                    <td style="width: 20px; text-align: center; vertical-align: top; padding-top: 10px;">
                        <div style="padding-left:5%; display: inline-block; width: 45%; vertical-align: top; text-align: left;">{{$letters[$i]}})</div>
                    </td>
                    <td>
                        {!! $question->answer_sheet_question_answer[$i]->answer_description !!}
                    </td>
                    @if(isset($question->answer_sheet_question_answer[$i + 1]))
                        <td style="width: 20px; text-align: center; vertical-align: top; padding-top: 10px; ">
                            <div style="padding-left:5%; display: inline-block; width: 40%; vertical-align: top; text-align: left;">{{$letters[$i+1]}})</div>
                        </td>
                        <td style="padding: 10px">
                            {!! $question->answer_sheet_question_answer[$i+1]->answer_description !!}
                        </td>
                    @endif
                </tr>
            @endfor
